...
class SharedMemoryObj:
    ...
    def WriteClear(self, clearPos = 0, esayclear = False):#clearPos: 0: all, 1-messages: messages shall be saved 
        ...
        if esayclear:
            ...
            shm_buffer[self.writeBufferStartPos + 15 : self.writeBufferStartPos + self.writebufferLength] = 0xFF
            ...
            shm_buffer[self.writeBufferStartPos + 9: self.writeBufferStartPos + 11] = IntToBytes(self.writtenMark)#write mark
            self.newMessageStartPos = self.writeBufferStartPos + 15 if len(self.messageStartPos) == 0 else self.messageStartPos[-1]
            self.newMessageEndPos = self.writeBufferStartPos + 15 + len(unreadMsg)
